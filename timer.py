# app.py (Final version with Gold-Standard Storytelling)
...
def generate_comment_story(topic, on_screen_text):
    """Generates a detailed, punchy, and highly engaging story in a specific list-based format."""
    prompt = f"""
    Context: A YouTube Short about "{topic}" was created with the on-screen text: "{on_screen_text}".
    Your Persona: You are a master storyteller uncovering the world's most fascinating and bizarre secrets. You sound like a real person sharing something mind-blowing, not an AI.
    Your Task: Write the pinned YouTube comment that reveals the full story.

    **CRITICAL STYLE RULES (NON-NEGOTIABLE):**
    1.  **START WITH A HOOK:** Begin with a short, intriguing paragraph that poses a question or makes a bold claim.
    2.  **USE A NUMBERED LIST:** The main body of your comment **MUST** be a numbered list (1., 2., 3., etc.). Each point should present a piece of evidence, a fact, or a report.
    3.  **PUNCHY SENTENCES:** Use very short, dramatic sentences for emphasis. Examples: "No explanation.", "Not a metaphor. Literal.", "Forget it."
    4.  **HUMAN TONE:** Use conversational language. Phrases like "Here's what's been reported...", "Get this:", "Still think it's just a mountain?".
    5.  **CITE "SOURCES":** Casually mention sources like "pilgrims," "ancient texts," "travelers," "scientists," "locals," or "satellite images" to add credibility.
    6.  **STRONG CONCLUSION:** End with a powerful concluding paragraph that challenges the reader or includes a memorable quote.
    Provide ONLY the text for the comment.

    ---
    **GOLD STANDARD EXAMPLE (THE EXACT STYLE YOU MUST FOLLOW)**
    Context: A Short about "Mount Kailash" with the text "Time Moves DIFFERENTLY Near Mount Kailash... *Read the comment for the proof*".

    **PERFECT COMMENT OUTPUT:**
    ...
    5. Satellite images show strange, unnatural symmetry at the summit, almost like it's designed - not formed.
    6. Compasses malfunction in the region. Phones and GPS? Forget it. Like the mountain rejects our tech.
...
if st.session_state.content:
    ...
    with st.container(border=True):
        ...
        if st.button("✍️ Generate Comment Story"):
            with st.spinner("✍️ Writing the full story..."):
